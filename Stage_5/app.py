...
import time  # הוספנו כדי לאפשר השהייה קטנה לפני ריענון מסך
...
def show_system_admin_page():
    ...
    # חלוקה ל-2 לשוניות: שאילתות (שלב ב/ג) ופרוצדורות (שלב ד')
    tab_queries, tab_procs = st.tabs(["📈 Advanced Analytics (Views)", "⚙️ System Operations (Procedures)"])

    # ==========================================
    # לשונית 1: שאילתות מתקדמות (שימוש ב-Views משלב ג')
    # ==========================================
    with tab_queries:
        ...
        with col2:
            ...
            if st.button("🏆 Show Tournaments Stats"):
                # ==========================================
                # הערה לבוחן: שימוש ב-VIEW במקום שאילתה מורכבת
                # כאן יישמתי את vw_tournament_stats. הקוד בפייתון נקי,
                # וה-DB עושה את ה-JOIN וה-GROUP BY מאחורי הקלעים.
                # ==========================================
                q2 = "SELECT * FROM vw_tournament_stats ORDER BY tournament_id;"
                df2 = db.run_query(q2)
                if df2 is not None:
            ...
            with st.container(border=True):
                ...
                if st.button("Calculate Score", type="primary"):
                    ...
                    if check_res is None or check_res.empty:
                        ...
                        # השתמשתי ב-SELECT כי פונקציה מחזירה ערך (בניגוד לפרוצדורה)
        ...
        with col4:
            with st.container(border=True):
                ...
                if st.button("Run Promotion Routine", type="primary"):
                    # השתמשתי ב-CALL בגלל שזו פרוצדורה שעושה UPDATE ולא מחזירה טבלה
...
def show_tournament_organizer_page():
    ...
    with tab1:
        ...
        # ==========================================
        # הערה לבוחן: שימוש ב-VIEW שהוכן מראש.
        # ה-vw_players_directory מחבר בין טבלת Player ל-Bot
        # ומפעיל COALESCE מבלי להעמיס על הפייתון.
        # ==========================================
        query_players = "SELECT * FROM vw_players_directory ORDER BY player_id DESC;"
        ...
        with st.form("add_player_form", clear_on_submit=True):
            ...
            if submit_add:
                if not new_username:
                    ...
                    # שימוש בשאילתה פרמטרית למניעת SQL Injection
                    insert_q = """
                    ...
                    if success:
                        ...
                        time.sleep(1) # ממתין שנייה כדי שהמשתמש יראה את ההודעה
                        st.rerun() # מרענן את האפליקציה אוטומטית
                    else:
    ...
    with tab3:
        ...
        if st.button("Fetch Player Details"):
            ...
            if player_data is not None and not player_data.empty:
                # שימוש ב-State כדי לשמור את הנתונים בזמן שהמשתמש מקליד
            ...
            with st.form("update_player_form"):
                ...
                if submit_upd:
                    ...
                    if success:
                        ...
                        del st.session_state['update_player_data'] 
                        time.sleep(1)
                        st.rerun() # הערה לבוחן: חוסך למשתמש ללחוץ F5 כדי לראות את השינוי בטבלה
                    else:
...
def show_technician_page():
    ...
    # --- READ (טלמטריה מתוך VIEW) ---
    with tab1:
        st.subheader("Latest Hardware Telemetry")
        # ==========================================
        # הערה לבוחן: עוד דוגמה לשימוש באבסטרקציה דרך VIEW.
        # ה-vw_live_telemetry מבצע את ה-JOIN בין Telemetry ל-Node.
        # ==========================================
        query = "SELECT * FROM vw_live_telemetry ORDER BY time_stamp DESC;"
    ...
    with tab3:
        ...
        with st.form("add_node_form", clear_on_submit=True):
            ...
            new_cpu_cores = st.number_input("CPU Cores Number", min_value=1, step=1, value=8) 
            new_ram = st.number_input("RAM (GB)", min_value=1, max_value=512, value=16)
            new_zone = st.text_input("Datacenter Zone (e.g., us-east-1)") 
            ...
            if submit_add:
                ...
                if success:
                    st.success(f"Node {new_ip} added successfully!")
                    time.sleep(1)
                    st.rerun()
                else:
